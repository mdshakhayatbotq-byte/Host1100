...
def create_app():
    ...
    @app.route('/files/content/<folder>/<path:name>')
    def fcontent(folder, name):
        sub_path = request.args.get('path', '').strip('/')
        full_path = safe_join(app.config['BASE_STORAGE'], folder, sub_path, name)
        
        if not full_path or not os.path.isfile(full_path):
            return jsonify({'content': 'Error: File not found or invalid path'}), 404
    ...
    def add_srv():
        ...
        # Create a default main.py so it doesn't throw file missing errors
